...
const CourseList = ({ groupid }: Props) => {
	...
	return data.courses?.map((course) => (
		<Link href={`/group/${groupid}/courses/${course.id}`} key={course.id}>
			<Card className="h-full overflow-hidden rounded-xl border-themeGray bg-transparent">
				<img
					src={`https://ucarecdn.com/${course.thumbnail}/`}
					alt="cover"
					className="h-4/6 w-full opacity-60"
				/>
				<div className="flex h-2/6 flex-col justify-center pl-5">
					<h2 className="font-semibold text-lg text-white">{course.name}</h2>
